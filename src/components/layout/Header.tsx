    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-[#0A0A0F]/90 backdrop-blur border-b border-[#1E1E2A]">
      <nav className="max-w-[1200px] mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex gap-8">
          {NAV_LINKS.map((l) => (
            <a key={l.href} href={l.href} className="font-body text-sm uppercase tracking-widest text-[#8A8F98] hover:text-[#E8ECF0] transition-colors">
              {l.label}
            </a>
          ))}
        </div>
      ...
      {open && (
        <div className="md:hidden bg-[#0A0A0F] border-t border-[#1E1E2A] px-6 py-4 flex flex-col gap-4">
          {NAV_LINKS.map((l) => (
            <a key={l.href} href={l.href} onClick={() => setOpen(false)} className="font-body text-sm uppercase tracking-widest text-[#8A8F98] hover:text-[#E8ECF0]">
              {l.label}
            </a>
          ))}
        </div>
